import React from "react";
import Header from "./Header";
import "./Home.css";
import Product from "./Product";
import CaroselBody from "./CaroselBody";

function Home() {
  return (
    <>
      <Header />
      <div className="home">
        {/* <img
          src="https://m.media-amazon.com/images/I/61jovjd+f9L._SX3000_.jpg"
          alt=""
          className="home__img"
        /> */}
        <CaroselBody />
        <div className="hdot"></div>
        <div className="home__row">
          <Product
            id="138472"
            title="The Lean Startup: The Lean Startup: How Today's Entrepreneurs Use Continuous Innovation to Create Radically Successful"
            price={15.59}
            rating={5}
            image="https://m.media-amazon.com/images/I/41Ag4WE7uyL._AC_UY218_.jpg"
          />
          <Product
            id="237863"
            title="KitchenAid KP26M1XER 6 Qt. Professional 600 Series Bowl-Lift Stand Mixer - Empire Red"
            price={15.59}
            rating={5}
            image="https://m.media-amazon.com/images/I/81BOGWDXGHL._AC_UY218_.jpg"
          />
        </div>
        <div className="home__row">
          <Product
            id="336726"
            title="PlayStation DualSense Wireless Controller – Galactic Purple"
            price={15.59}
            rating={5}
            image="https://m.media-amazon.com/images/I/61Uh8NFDzsL._AC_UY218_.jpg"
          />
          <Product
            id="437263"
            title="Apple iPhone 13 Pro Max, 128GB, Graphite - Unlocked (Renewed)"
            price={499.59}
            rating={5}
            image="https://m.media-amazon.com/images/I/61iVuXR3dhL._AC_UY218_.jpg"
          />
          <Product
            id="5392736"
            title="2021 Apple 11-inch iPad Pro (Wi-Fi, 128GB) - Silver"
            price={15.59}
            rating={5}
            image="https://m.media-amazon.com/images/I/81MF389-9gS._AC_UY218_.jpg"
          />
        </div>
        <div className="home__row">
          <Product
            id="6362561"
            title="SAMSUNG CJ890 Series 49-Inch Ultrawide QHD (3840x1080) Computer Monitor, 144Hz, Curved, HDMI, USB-C, Height Adjustable Stand, 3 Yr WRNTY (LC49J890DKNXZA)"
            price={15.59}
            rating={4}
            image="https://m.media-amazon.com/images/I/81B0xCF+PWL._AC_SX679_.jpg"
          />
        </div>
      </div>
    </>
  );
}

export default Home;
